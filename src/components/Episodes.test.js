...
test('Episodes component renders correctly with empty array passed to it', () => {
    const { queryAllByTestId} = render(<Episodes episodes={[]}/>)

    expect(queryAllByTestId('episode')).toHaveLength(0);
})

test('Episodes component renders correctly with array passed to it', () => {
    const { queryAllByTestId} = render(<Episodes episodes={episodeArray}/>)
